//
// Created on 13/02/24 using Swift 5.0
...
class EssentialFeedCacheIntegrationTests: XCTestCase {
    ...
    func test_load_deliversNoItemsOnEmptyCache() {
        let feedLoader = makeLocalFeedLoader()

        expect(feedLoader, toLoad: [])
    }
    
    func test_load_deliversItemsSavedOnASeparateInstance() {
        let feedLoaderToPerformSave = makeLocalFeedLoader()
        let feedLoaderToPerformLoad = makeLocalFeedLoader()
    ...
    func test_save_overridesItemsSavedOnASeparateInstance() {
        let feedLoaderToPerformFirstSave = makeLocalFeedLoader()
        let feedLoaderToPerformLastSave = makeLocalFeedLoader()
        let feedLoaderToPerformLoad = makeLocalFeedLoader()
    ...
    func test_validateFeedCache_doesNotDeleteRecentlySavedFeed() {
        let feedLoaderToPerformSave = makeLocalFeedLoader()
        let feedLoaderToPerformValidation = makeLocalFeedLoader()
        let feed = uniqueImageFeed().model
    ...
    func test_validateFeedCache_deletesFeedSavedInADistantPast() {
        let feedLoaderToPerformSave = makeLocalFeedLoader(currentDate: .distantPast)
        let feedLoaderToPerformValidation = makeLocalFeedLoader(currentDate: Date())
        let feed = uniqueImageFeed().model
    ...
    func test_loadImageData_deliversSavedDataOnASeparateInstance() {
        let imageLoaderToPerformSave = makeLocalFeedImageDataLoader()
        let imageLoaderToPerformLoad = makeLocalFeedImageDataLoader()
        let feedLoader = makeLocalFeedLoader()
    ...
    func test_saveImageData_overridesSavedImageDataOnASeparateInstance() {
        let imageLoaderToPerformFirstSave = makeLocalFeedImageDataLoader()
        let imageLoaderToPerformLastSave = makeLocalFeedImageDataLoader()
        let imageLoaderToPerformLoad = makeLocalFeedImageDataLoader()
        let feedLoader = makeLocalFeedLoader()
        let image = uniqueImage()
    ...
    private func makeLocalFeedLoader(currentDate: Date = Date(), file: StaticString = #file, line: UInt = #line) -> LocalFeedLoader {
    ...
    private func makeLocalFeedImageDataLoader(file: StaticString = #file, line: UInt = #line) -> LocalFeedImageDataLoader {
        let storeURL = testSpecificStoreURL()
        let store = try! CoreDataFeedStore(storeUrl: storeURL)
